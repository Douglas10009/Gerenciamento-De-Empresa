...
public class SeachDelete {
    ...
    public static void apagarCnpj(int cnpj, ArrayList<Fornecedores> list_forn, boolean cad_Fornecedor) {
    	...
    	System.out.println("Posição do CNPJ: " + posicao); //Degub
    ...
    public static void apagarCodigoFornecedor(int cod_fornecedor, ArrayList<Fornecedores> list_forn, boolean cad_Fornecedor) {
    	int posicao = buscarCodigoFornecedor(cod_fornecedor, list_forn);
    	System.out.println("Posição do código do fornecedor: " + posicao); //Degub
    	
    	if (posicao != -1) {
			list_forn.remove(posicao);
			System.out.println("\n-----> Fornecedor com o código " + posicao + " foi apagado com sucesso."); //ALERT
            if (list_forn.size() < 1) {
                cad_Fornecedor = false;
            } 
